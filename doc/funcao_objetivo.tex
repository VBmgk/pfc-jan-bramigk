...
\begin{dmath} 
 c_{abertura{\ }gol} = taxa_{abertura{\ }total/max{\ }gol} . \sum abertura{\ }gol(r_i)
 + (1 - taxa_{abertura{\ }total/max{\ }gol}) . max \lbrace abertura{\ }gol(r_i) \rbrace 
\end{dmath} 
...
\begin{dmath} 
 c_{dist{\ }total{\_}mover} = p_{dist{\ }total{\_}mover} . 
...
% TODO: Explicar notação
\begin{dmath} 
 c_{ataque} = p_{ataque} . abertura{\ }gol_ad
\end{dmath} 

Onde $enemy{\_}gap_{value}$ é o valor da abertura do gol adversário visto
...
\subsection{Custo do bloqueio do gol}
