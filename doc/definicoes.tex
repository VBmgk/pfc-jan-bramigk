...
\begin{defi}[Corpo Rígido]
  ...
  \begin{itemize}
    \item $\hat{cr} = \langle \alpha, \beta, \gamma, \omega \rangle$,
...
% TODO: referenciar secção do capítulo da arquitetura que fala da camera
% TODO: referenciar imagem da secção Representação do jogo
  Conforme citado em na secção (referenciar), $\delta t$ normalmente é $16,6{\ }ms$.
  Um exemplo de $A^{i}$ pode ser visto na figura~\ref{fig:acoes_time}.
...
\begin{defi}[Logs]
  ...
  % TODO: Citar trabalho do CRC
  %em (\textbf{citar trabalho CRC}).
\end{defi}
