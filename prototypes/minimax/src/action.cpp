...
Action Action::genMove(const Robot &r, Player p, const Board &b,
                       const MoveTable &mtable) {
  Vector pos;
  bool ok = true;
  float rand_r;
...
again:
  ...
  if ((b.goalPos(p) - pos).norm() <= b.goalDefense())
    goto again;

  auto ball = b.getBall();
  if (ball.getDist(pos) <= r.radius() + ball.radius())
    goto again;
  ...
  for (auto mov : mtable) {
    if (mov.first != r.getId()) {
      if (mov.second.type == MOVE &&
          (pos - mov.second.move_point).norm() <=
              r.radius() + b.getBall().radius())
        goto again;
    }
  }
...
void applyAction(const Action &a, Player p, Board &b) {
  ...
  case PASS: {
    ...
    if (rcv != nullptr) {
      ...
      std::cerr << "XXX pass_id " << a.pass_id
                << " not found... (robot_id = " << a.robot_id << ")"
                << std::endl;
    }
  } break;
  default: {}
