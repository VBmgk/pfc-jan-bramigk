 ...
 **********************************************************/
tuple<vec, num> grad_min(num (*f)(const vec), const vec x_init, const num gamma) {
  vec x_k(x_init.size()), x_buff(x_init.size());
  unsigned int n_it = 1; num erro = 1;
  ...
  while (n_it < MAX_IT && (erro > TOL)) {
    ...
    if (norm(x_buff) > MAX_VALUE) { x_buff *= 0.0; }
...
int eig(const mat& _A, num* eigen_values) {
  ...
  if (_A.size1() == _A.size2()) {
    // allocate memory
    A             = new num[n * n],
    work_array    = new num[size_work_array];
    // free memory
    copy(_A.data().begin(), _A.data().end(), A);
    // call lapack subrotine
    // XXX: This only works with double!!!
    LAPACK_dsyev (&return_eigen_vectors, &upper_lower_triag,
                  &order_of_A, A, &n,
                  eigen_values,
                  work_array, &size_work_array, &info);
  } else { info = -1; }
  // free memory
  delete A;
  delete work_array;
...
tuple<num,num,int> max_min_eigs(const mat& A) {
  num *eigenvalues = new num[A.size1()],
      max = 0,
      min = 0;
  int info = -1;
...
tuple<vec,num,num,num,int> newton_min(num (*f)(const vec), vec x) {
  ...
  while (n_it < MAX_IT && erro > TOL) {
    ...
    // One need to analize the eigen values to find the type of inflexion point
 ...
 ***************************************************************/
tuple<vec,num>
 newton_grad(num (*f)(const vec), const vec x_0,
             const num gamma, const bool find_minimum) {
  // initial values
  ...
  while (n_it < MAX_IT && erro > TOL) {
    n_sub_it = 1; n_it += 1;
    // First, get out of inflexion points, because the gradient may increase
    // fast in that points. For this to be done, it is necessary to discover
